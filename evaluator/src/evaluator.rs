...
use crate::helpers::get_variable_span;
use crate::{
    environment::Environment,
    php_type::{ErrorLevel, PhpError, PhpValue},
};
...
pub struct Evaluator {
    ...
    /// The error (usually a fatal error)
    pub error: PhpError,
...
impl Evaluator {
    ...
    pub fn eval_statement(&mut self, node: Statement) {
        match node {
            ...
            Statement::Echo(echo) => {
                for expr in echo.values {
                    let expression_value = self.eval_expression(expr);

                    self.output += expression_value.to_string().as_str();
                }
            }
            _ => {
    ...
    fn eval_expression(&mut self, expr: Expression) -> PhpValue {
        match expr {
            ...
            Expression::Empty(ee) => {
                ...
                match arg {
                    Argument::Named(na) => {
                        ...
                        self.die_with_message(error, ErrorLevel::Error);
                    ...
                    match arg {
                        ...
                        Argument::Positional(pa) => {
                            if let Expression::Variable(va) = pa.value {
                                ...
                                self.env.delete(var_name.as_str());
                            } else {
                                let error = format!(
            ...
            Expression::Print(pe) => {
                if pe.value.is_some() {
                    ...
                    self.output += value.to_string().as_str();
            ...
            Expression::ComparisonOperation(operation) => match operation {
                ComparisonOperationExpression::Equal { left, right, .. } => {
                    ...
                    PhpValue::Bool(left_value == right_value)
                }
            ...
            Expression::LogicalOperation(operation) => match operation {
                LogicalOperationExpression::And { left, right, .. } => {
                    ...
                    PhpValue::Bool(left_value.is_true() && right_value.is_true())
                }
                ...
                LogicalOperationExpression::Not { right, .. } => {
                    ...
                    PhpValue::Bool(!right_value.is_true())
                }
            ...
            Expression::ErrorSuppress(error_expression) => {
                let php_die = self.die;
                let warnings = self.warnings.clone();

                self.eval_expression(*error_expression.expr);

                if php_die != self.die || warnings.len() != self.warnings.len() {}

                PhpValue::Null
            }
            Expression::Bool(b) => PhpValue::Bool(b.value),
            _ => PhpValue::Null,
        }
    }
    ...
    fn die_with_message(&mut self, message: String, level: ErrorLevel) {
        self.error = PhpError { level, message };
        self.die();
    }

    fn eval_error(&mut self, error: PhpError) {
        match error.level {
            ErrorLevel::Error => {
                self.error = error;
                self.die();
    ...
    fn php_value_or_die(&mut self, span: Span, value: Result<PhpValue, PhpError>) -> PhpValue {
        match value {
            Ok(value) => value,
            Err(error) => {
                self.die_with_message(
                    error.message + format!(" on line: {}", span.line).as_str(),
                    ErrorLevel::Error,
                );
                PhpValue::Null
            }
    ...
    fn get_variable_name(&mut self, variable: Variable) -> String {
        match variable {
            Variable::SimpleVariable(sv) => sv.name.to_string(),
            ...
            Variable::BracedVariableVariable(_bvv) => todo!(),
        }
    }

    fn get_variable_value(&mut self, variable: Variable) -> PhpValue {
        match variable {
            Variable::SimpleVariable(sv) => {
                let var_name = sv.name.to_string();

                let value = self.env.get(&var_name);
    ...
    fn change_var_value(
        ...
        let current_var_value = self.env.get(&var_name);
        ...
        if !self.env.exists(&var_name) {
            ...
            let old_value = Rc::clone(self.env.get_var_with_rc(&var_name).unwrap());
    ...
    fn get_var(&mut self, variable: Variable) -> PhpValue {
    ...
    fn assign(&mut self, left: Expression, span: Span, right: Expression) -> PhpValue {
        ...
        if let Expression::Reference(reference) = right {
            ...
            if !self.env.exists(&right_var_name) {
                self.env.set(&right_var_name, PhpValue::Null)
            }
            ...
            self.env.set_rc(&left_var_name, Rc::clone(right_value));

            return right_value.borrow().clone();
        } else {
            let right_value = self.eval_expression(right);

            let right_value_clone = right_value.clone();

            if !self.env.exists(&left_var_name) {
                self.env.set(&left_var_name, right_value_clone);
            } else {
                let old_value = Rc::clone(self.env.get_var_with_rc(&left_var_name).unwrap());
