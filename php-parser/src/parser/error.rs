...
use self::ariadne::{CharSet, Color, Config, Label, Report, ReportKind, Source};
...
pub fn cannot_use_positional_argument_after_argument_unpacking(
    span: Span,
    current_span: Span,
) -> ParseError {
    ParseError::new(
...
pub fn only_one_argument_is_accepted(span: Span, current_span: Span) -> ParseError {
    ParseError::new("E051".to_string(), "only one argument are accepted", span).error(
...
pub fn argument_is_required(span: Span, current_span: Span) -> ParseError {
    ParseError::new("E052".to_string(), "argument is required", span).error(
